...
class TraumaPage extends StatefulWidget {
  ...
  @override
  State<TraumaPage> createState() => _TraumaPageState();
}
...
class TraumaPanel extends StatefulWidget {
  const TraumaPanel({Key? key}) : super(key: key);

  @override
  State<TraumaPanel> createState() => _TraumaPanelState();
}

class _TraumaPanelState extends State<TraumaPanel> {
  ...
  void update() {
    context.read<PostList>().updateTraumaPostList();
  }
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      context.read<PostList>().searchTraumaPostList(Search.text);
                    },
                    child: Text("검색"),
                  ),
                ),
          ...
          child: Consumer<PostList>(
            builder: (context, postList, child) {
              return ListView.builder(
                itemCount: postList.allTraumaList.length,
                itemBuilder: (context, index) {
                  return Padding(
                    ...
                    child: Container(
                      ...
                      child: Card(
                        child: ListTile(
                          title: Text(postList.allTraumaList[index].title, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis,),
                          subtitle: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(postList.allTraumaList[index].question, style: TextStyle(fontSize: 18), maxLines: 2, overflow: TextOverflow.ellipsis,),
                              Row(
                                children: [
                                  Text(postList.allTraumaList[index].category, style: TextStyle(color: Colors.grey),),
                                  Text(" | "),
                                  Text(
                                    postList.allTraumaList[index].answer == null ? "답변 전" : "답변 완료",
                          ...
                          onTap: () {
                            showModalBottomSheet<void>(
                              ...
                              builder: (context) => Container(
                                height: MediaQuery.of(context).size.height * 0.9,
                                child: PostDetail(post: postList.allTraumaList[index]),
                              ),
                            );
                          },
                        ),
            ...
            ElevatedButton(
              onPressed: () async {
                if(TitleController.text != "" && QuestionController.text != "") {
                  Post savePost = Post(category: 'Trauma', title: TitleController.text, question: QuestionController.text, docName: null, answer: null);
