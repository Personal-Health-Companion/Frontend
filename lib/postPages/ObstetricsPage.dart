...
class ObstetricsPanel extends StatefulWidget {
  const ObstetricsPanel({Key? key}) : super(key: key);

  @override
  State<ObstetricsPanel> createState() => _ObstetricsPanelState();
}

class _ObstetricsPanelState extends State<ObstetricsPanel> {
  ...
  void update() {
    context.read<PostList>().updateObstetricsPostList();
  }
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      context.read<PostList>().searchObstetricsPostList(Search.text);
                    },
                    child: Text("검색"),
                  ),
                ),
          ...
          child: Consumer<PostList>(
            builder: (context, postList, child) {
              return ListView.builder(
                itemCount: postList.allObstetricsList.length,
                itemBuilder: (context, index) {
                  return Padding(
                    ...
                    child: Container(
                      ...
                      child: Card(
                        child: ListTile(
                          title: Text(postList.allObstetricsList[index].title, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis,),
                          subtitle: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(postList.allObstetricsList[index].question, style: TextStyle(fontSize: 18), maxLines: 2, overflow: TextOverflow.ellipsis,),
                              Row(
                                children: [
                                  Text(postList.allObstetricsList[index].category, style: TextStyle(color: Colors.grey),),
                                  Text(" | "),
                                  Text(
                                    postList.allObstetricsList[index].answer == null ? "답변 전" : "답변 완료",
                          ...
                          onTap: () {
                            showModalBottomSheet<void>(
                              ...
                              builder: (context) => Container(
                                height: MediaQuery.of(context).size.height * 0.9,
                                child: PostDetail(post: postList.allObstetricsList[index]),
                              ),
                            );
                          },
                        ),
            ...
            ElevatedButton(
              onPressed: () async {
                if(TitleController.text != "" && QuestionController.text != "") {
                  Post savePost = Post(category: 'Obstetrics', title: TitleController.text, question: QuestionController.text, docName: null, answer: null);
