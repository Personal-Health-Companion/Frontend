...
class DentistryPanel extends StatefulWidget {
  const DentistryPanel({Key? key}) : super(key: key);

  @override
  State<DentistryPanel> createState() => _DentistryPanelState();
}

class _DentistryPanelState extends State<DentistryPanel> {
  ...
  void update() {
    context.read<PostList>().updateDentistryPostList();
  }
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      context.read<PostList>().searchDentistryPostList(Search.text);
                    },
                    child: Text("검색"),
                  ),
                ),
          ...
          child: Consumer<PostList>(
            builder: (context, postList, child) {
              return ListView.builder(
                itemCount: postList.allDentistryList.length,
                itemBuilder: (context, index) {
                  return Padding(
                    ...
                    child: Container(
                      ...
                      child: Card(
                        child: ListTile(
                          title: Text(postList.allDentistryList[index].title, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis,),
                          subtitle: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(postList.allDentistryList[index].question, style: TextStyle(fontSize: 18), maxLines: 2, overflow: TextOverflow.ellipsis,),
                              Row(
                                children: [
                                  Text(postList.allDentistryList[index].category, style: TextStyle(color: Colors.grey),),
                                  Text(" | "),
                                  Text(
                                    postList.allDentistryList[index].answer == null ? "답변 전" : "답변 완료",
                          ...
                          onTap: () {
                            showModalBottomSheet<void>(
                              ...
                              builder: (context) => Container(
                                height: MediaQuery.of(context).size.height * 0.9,
                                child: PostDetail(post: postList.allDentistryList[index]),
                              ),
                            );
                          },
                        ),
            ...
            ElevatedButton(
              onPressed: () async {
                if(TitleController.text != "" && QuestionController.text != "") {
                  Post savePost = Post(category: 'Dentistry', title: TitleController.text, question: QuestionController.text, docName: null, answer: null);
