...
class InternalMedicinePage extends StatefulWidget {
  const InternalMedicinePage({Key? key}) : super(key: key);

  @override
  State<InternalMedicinePage> createState() => _InternalMedicinePageState();
}

class _InternalMedicinePageState extends State<InternalMedicinePage> {
  @override
...
class InternalMedicinePanel extends StatefulWidget {
  const InternalMedicinePanel({Key? key}) : super(key: key);

  @override
  State<InternalMedicinePanel> createState() => _InternalMedicinePanelState();
}

class _InternalMedicinePanelState extends State<InternalMedicinePanel> {
  ...
  void update() {
    context.read<PostList>().updateInternalMedicinePostList();
  }
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: ElevatedButton(
                    onPressed: () {
                      context.read<PostList>().searchInternalMedicinePostList(Search.text);
                    },
                    child: Text("검색"),
                  ),
                ),
          ...
          child: Consumer<PostList>(
            builder: (context, postList, child) {
              return ListView.builder(
                itemCount: postList.allInternalMedicineList.length,
                itemBuilder: (context, index) {
                  return Padding(
                    ...
                    child: Container(
                      ...
                      child: Card(
                        child: ListTile(
                          title: Text(postList.allInternalMedicineList[index].title, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold), overflow: TextOverflow.ellipsis,),
                          subtitle: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(postList.allInternalMedicineList[index].question, style: TextStyle(fontSize: 18), maxLines: 2, overflow: TextOverflow.ellipsis,),
                              Row(
                                children: [
                                  Text(postList.allInternalMedicineList[index].category, style: TextStyle(color: Colors.grey),),
                                  Text(" | "),
                                  Text(
                                    postList.allInternalMedicineList[index].answer == null ? "답변 전" : "답변 완료",
                          ...
                          onTap: () {
                            showModalBottomSheet<void>(
                              ...
                              builder: (context) => Container(
                                height: MediaQuery.of(context).size.height * 0.9,
                                child: PostDetail(post: postList.allInternalMedicineList[index]),
                              ),
                            );
                          },
                        ),
            ...
            ElevatedButton(
              onPressed: () async {
                if(TitleController.text != "" && QuestionController.text != "") {
                  Post savePost = Post(category: 'InternalMedicine', title: TitleController.text, question: QuestionController.text, docName: null, answer: null);
